...
void ParticleWindow::UpdateSieveChart()
{
...
void ParticleWindow::LoadCalImage(const entry_ref& ref)
{
    BBitmap* bmp = BTranslationUtils::GetBitmapFile(BPath(&ref).Path());
    if (!bmp || !bmp->IsValid()) {
        delete bmp;
...
void ParticleWindow::AnalyseCal()
{
...
void ParticleWindow::UpdateCalSubMethod()
{
...
void ParticleWindow::RunCalibrate()
{
    if (!fCalSource || !fCalSource->IsValid()) return;
...
float ParticleWindow::DetectCirclePxMm(BBitmap* bmp, BRect normSel, float diamMm)
{
...
float ParticleWindow::DetectLinesPxMm(BBitmap* bmp, BRect normSel, float spacingMm)
{
    if (!bmp || !bmp->IsValid()) return 0.0f;
...
void ParticleWindow::UpdateUnits()
{
    const char* unit = CoffeeSettings::Get()->UseMetric() ? "mm" : "in";
...
void ParticleWindow::MessageReceived(BMessage* msg)
{
    switch (msg->what) {
